...
export default component$(({ charge, destination, trajets }: { charge: string, destination: string, trajets: any }) => {
  ...
  useTask$(({ track }) => {
    ...
    if (charge && destination) {
      const index = trajets.findIndex((trajet: any) =>strUcFirst(trajet.destination).includes(strUcFirst(destination)) && strUcFirst(trajet.depart).includes(strUcFirst(charge)))
  ...
  return (
    ...
    <>
      <div class="flex flex-col bg-white mt-[10px]  mx-auto max-w-6xl w-full">        
        {/* InfoOffre; */}
        {destination && (
        ...
        <div class="flex flex-col sm:flex-col  md:flex-col lg:flex-row gap-5">
          ...
          {destination && strUcFirst(destination) === 'Orly' && maps['Orly']()}
          {destination && strUcFirst(destination) === 'Paris' && maps['Paris']()}
          {destination && strUcFirst(destination) === 'Disneyland' && maps['Disneyland']()}
          {destination && strUcFirst(destination) === 'Beauvais' && maps['Beauvais']()}
          {destination && strUcFirst(destination) === 'Cdg' && maps['Cdg']()}
          {/* if theres NO a destination, render the charge map */}
          {!destination && strUcFirst(charge) === 'Orly' && maps['Orly']()}
          {!destination && strUcFirst(charge) === 'Paris' && maps['Paris']()}
          {!destination && strUcFirst(charge) === 'Disneyland' && maps['Disneyland']()}
          {!destination && strUcFirst(charge) === 'Beauvais' && maps['Beauvais']()}
          {!destination && strUcFirst(charge) === 'Cdg' && maps['Cdg']()}
        </div>
